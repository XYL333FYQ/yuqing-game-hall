...
bkcore.hexgl.HexGL = function(opts)
{
	...
	// Keep the game's cameras, HUD and input in full CSS pixels, but avoid feeding millions of pixels per
	// frame through this 2013 post stack on an integrated GPU. One megapixel is a stable starting budget;
	// discrete/unknown adapters keep native resolution. CSS upscales only the 3D canvas.
	var integrated = document.documentElement.dataset.gpuClass === 'integrated';
	var pixelBudget = 1500000;
	this.renderScale = integrated ? Math.min(1, Math.max(0.4, Math.sqrt(pixelBudget / (this.width * this.height)))) : 1;
	this.renderWidth = Math.max(1, Math.round(this.width * this.renderScale));
	this.renderHeight = Math.max(1, Math.round(this.height * this.renderScale));
	document.documentElement.dataset.renderScale = Math.round(this.renderScale * 100);
...
bkcore.hexgl.HexGL.prototype.initGameComposer = function()
{
	...
	var renderTarget = new THREE.WebGLRenderTarget( this.renderWidth, this.renderHeight, renderTargetParameters );
	...
	effectHex.uniforms[ 'size' ].value = 512.0 * (this.renderWidth/1633);
	effectHex.uniforms[ 'rx' ].value = this.renderWidth;
	effectHex.uniforms[ 'ry' ].value = this.renderHeight;
