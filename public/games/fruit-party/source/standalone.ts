...
root.innerHTML = `
  <main class="fruit-game">
    <header class="toolbar"><a href="?route=home" class="brand">🍉 <strong>果切派对</strong></a><div class="mode-switch"><button data-mode="arcade" class="active">90 秒街机</button><button data-mode="endless">经典无尽</button></div><div class="tools"><button data-action="mute">声音</button><button data-action="pause">暂停</button><button data-action="fullscreen">全屏</button></div></header>
...
function notifyHostReady(): void {
  window.parent !== window && window.parent.postMessage(
    { type: "YUQING_GAME_READY", gameId: "fruit-party" },
