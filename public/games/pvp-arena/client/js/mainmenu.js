...
function MainMenu() {
	...
	this.initialize=function() {
		...
		var options=[
			{label:"本地多人",id:GAMESTATE_LOCALMULTIPLAYER}
		];
		if (NETPLAY.isAvailable)
			options.push({label:"在线多人",id:GAMESTATE_NETPLAY});
		options.push({label:"单人街机",id:GAMESTATE_SCOREATTACK});
		options.push({label:"游戏设置",id:GAMESTATE_SETTINGS});
		options.push({label:"制作名单",id:GAMESTATE_CREDITS});
		if (KIOSKMODE.isKioskMode())
			options.push({label:"退出",id:GAMESTATE_QUIT});
		if (PLAYLIGHT.isPlayLightMode())
			options.push({label:"更多游戏",id:GAMESTATE_MOREGAMES});
