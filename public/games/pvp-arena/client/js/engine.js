...
function Canvas() {
	...
	this.printCenter=function(ctx,font,color,x,y,text,flipx,flipy,rotate,opacity,scale,composite) {
		text=text+"";
		if (window.yuqingTranslateText) text=window.yuqingTranslateText(text);
		scale=scale||1;
	...
	this.pixelStrLen=function(font,str) {
		...
		if (/[^\x20-\x7e]/.test(str)) {
			var width=0;
			for (var i=0;i<str.length;i++) width+=str.charCodeAt(i)>127?font.tileHeight:font.tileWidthSpaced;
	...
	this.print=function(ctx,font,color,x,y,text,flipx,flipy,rotate,opacity,scale,composite) {
		...
		text=text+"";
		if (window.yuqingTranslateText) text=window.yuqingTranslateText(text);
		// The original bitmap font only contains printable ASCII. Rainfine Game Hall
		// uses the browser font for localized text so Chinese menus stay readable
		// without changing the crisp bitmap rendering used by the game HUD.
		if (/[^\x20-\x7e]/.test(text)) {
			var nativePalette=["#1f0e1c","#34859d","#647d34","#7ec4c1","#9d303b","#70377f","#c0c741","#f5edba"];
			ctx.save();
			ctx.globalAlpha=opacity===undefined?1:opacity;
			ctx.fillStyle=nativePalette[color]||nativePalette[7];
			ctx.font="700 "+Math.max(8,QMATH.floor(font.tileHeight*scale))+"px 'Microsoft YaHei UI','PingFang SC',sans-serif";
			ctx.textBaseline="top";
			ctx.fillText(text,QMATH.floor(x),QMATH.floor(y));
			ctx.restore();
			return;
		}
