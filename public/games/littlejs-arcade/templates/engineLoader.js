...
'use strict';
{
    ...
    // Every standalone cabinet gets the same local favicon and the game-hall
    // localization/ready bridge.  Both URLs stay relative to this checked-in
    // loader, so they also work when the whole folder is deployed below a path.
    if (!document.querySelector('link[rel~="icon"]'))
    {
        const icon = document.createElement('link');
        icon.rel = 'icon';
        icon.type = 'image/png';
        icon.href = new URL('../images/favicon.png', loaderSource).href;
        document.head.appendChild(icon);
    }
    const hallBridge = new URL('../../_shared/yuqing-bridge.js', loaderSource).href;
    ...
    document.write(`<script>
        ...
        // Localize canvas-drawn HUD/status text as well as the DOM menus. The
        // cached translator adds negligible work, and Chinese strings use a
        // system font so they never turn into missing-glyph squares.
        const yuqingOriginalDrawTextScreen = drawTextScreen;
        drawTextScreen = function(text, ...args) {
            const localized = typeof text === 'string' && window.yuqingTranslateText
                ? window.yuqingTranslateText(text) : text;
            if (typeof localized === 'string' && /[\u3400-\u9fff]/.test(localized) &&
                (!args[6] || args[6] === 'monospace'))
                args[6] = '"Microsoft YaHei UI", sans-serif';
            return yuqingOriginalDrawTextScreen(localized, ...args);
        };
    <\/script>`);
}
