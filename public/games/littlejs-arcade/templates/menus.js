...
// Yuqing Game Hall localization layer. Most LittleJS prototypes share this
// helper, so common menus can be translated without rewriting each game.
const YUQING_MENU_ZH = new Map([
    ['PLAY', '开始游戏'],
    ['START', '开始游戏'],
    ['START GAME', '开始游戏'],
    ['NEW GAME', '新游戏'],
...
function localizeMenuText(value)
{
    if (typeof value !== 'string') return value;
    const trimmed = value.trim();
    if (!trimmed) return value;

    let translated = YUQING_MENU_ZH.get(trimmed.toUpperCase());
    if (!translated)
    {
        const metricPatterns = [
            [/^Best\s*:\s*(.*)$/i, '最佳：'],
            [/^High Score\s*:\s*(.*)$/i, '最高分：'],
            [/^Score\s*:\s*(.*)$/i, '得分：'],
            [/^Level\s*:\s*(.*)$/i, '关卡：'],
            [/^Time\s*:\s*(.*)$/i, '时间：'],
            [/^Lives\s*:\s*(.*)$/i, '生命：'],
        ];
        for (const [pattern, prefix] of metricPatterns)
        {
            const match = trimmed.match(pattern);
            if (match) { translated = prefix + match[1]; break; }
        }
    }
    ...
    if (!translated)
    {
        ...
        if (levelMatch) translated = '第 ' + levelMatch[1] + ' 关';
    }

    if (!translated)
    {
        const bestLevelMatch = trimmed.match(/^BEST\s+(?:LV|LEVEL)\s+(\d+)$/i);
        if (bestLevelMatch) translated = '最高关卡：' + bestLevelMatch[1];
    }

    if (!translated)
    {
        const compactMetric = trimmed.match(/^(BEST|HIGH SCORE|HI-SCORE|SCORE|LEVEL|TIME|LIVES|WAVE)\s+(-?\d[\d.:]*(?:\s*[A-Za-z]+)?)$/i);
        if (compactMetric)
        {
            const labels = {
                'BEST': '最佳', 'HIGH SCORE': '最高分', 'HI-SCORE': '最高分',
                'SCORE': '得分', 'LEVEL': '关卡', 'TIME': '时间',
                'LIVES': '生命', 'WAVE': '波次',
            };
            let metricValue = compactMetric[2];
            metricValue = metricValue.replace(/\s*M$/i, ' 米');
            translated = labels[compactMetric[1].toUpperCase()] + '：' + metricValue;
        }
    }
...
function createMenu(config)
{
    ...
    if (cfg.title)
    {
        ...
        titleEl.textContent = localizeMenuText(cfg.title);
    ...
    if (cfg.subtitle)
    {
        ...
        s.textContent = localizeMenuText(cfg.subtitle);
        panel.appendChild(s);
    }
...
function processToastQueue(position)
{
    ...
    if (opts.title)
    {
        ...
        titleEl.textContent = localizeMenuText(opts.title);
    ...
    if (opts.text)
    {
        ...
        textEl.textContent = localizeMenuText(opts.text);
...
function updateOrientationLock()
{
    ...
    if (blocked)
    {
        ...
        el.querySelector('.ljs-orient-title').textContent = localizeMenuText(
            orientationOpts.title || '请旋转设备');
        el.querySelector('.ljs-orient-text').textContent = localizeMenuText(orientationOpts.text) ||
            (orientationLock === 'landscape' ? '请将设备横屏使用。' : '请将设备竖屏使用。');
...
function buildGrid(item)
{
    ...
    cells.forEach((cell, i) =>
    {
        ...
        if (tip) cellEl.title = localizeMenuText(tip);
        ...
        labelEl.textContent = localizeMenuText(cell.label || '');
    ...
    const handle = {
        ...
        setCell(index, props)
        {
            ...
            if (props.label !== undefined) cellEl.querySelector('.ljs-grid-label').textContent = localizeMenuText(props.label);
        },
...
function buildText(item)
{
    ...
    el.textContent = localizeMenuText(item.text || '');
    if (item.title) el.title = localizeMenuText(item.title);
    const handle = {
        type: 'text',
        setLabel(t)    { el.textContent = localizeMenuText(t); },
        setValue()     {},
        getValue()     { return undefined; },
...
function buildButton(item)
{
    ...
    el.className = 'ljs-menu-item';
    el.textContent = localizeMenuText(item.label || '');
    if (item.title) el.title = localizeMenuText(item.title);
...
function buildToggle(item)
{
    ...
    let baseLabel = localizeMenuText(item.label || '');
    const render = () => { el.textContent = baseLabel + '：' + (value ? '开启' : '关闭'); };
    render();
    wireActivate(el, () =>
    {
        value = !value;
    ...
    const handle = {
        type: 'toggle',
        setLabel(t)    { baseLabel = localizeMenuText(t); render(); },
...
function buildSlider(item)
{
    ...
    if (item.title) wrap.title = localizeMenuText(item.title);

    const labelRow = document.createElement('div');
    labelRow.className = 'ljs-slider-row';
    const labelEl = document.createElement('span');
    labelEl.textContent = localizeMenuText(item.label || '');
    ...
    slider.type = 'range';
    if (item.title) slider.title = localizeMenuText(item.title);
    ...
    const handle = {
        type: 'slider',
        setLabel(t)    { if (t instanceof Node) labelEl.replaceChildren(t); else labelEl.textContent = localizeMenuText(t); },
...
function buildInput(item)
{
    ...
    if (item.placeholder) input.placeholder = localizeMenuText(item.placeholder);
    if (item.maxLength) input.maxLength = item.maxLength;
    if (item.title) input.title = localizeMenuText(item.title);
...
function buildToolbarLabelSpan(text)
{
    ...
    else span.textContent = localizeMenuText(text || '');
    return span;
}
...
function buildToolbarButton(item)
{
    ...
    const handle = {
        type: 'button',
        setLabel(t)    { if (t instanceof Node) labelEl.replaceChildren(t); else labelEl.textContent = localizeMenuText(t); },
        setTitle(t)    { el.title = localizeMenuText(t || ''); },
        setValue()     {},
        getValue()     { return undefined; },
